...
@Entity(tableName = "sessions", indices = [Index("profileId"), Index("phase")]) data class SessionRow(@PrimaryKey val sessionId: String, val profileId: String, val phase: String, val inputRevision: Long, val stateVersion: Long, val createdAtMs: Long, val summaryVersion: Int, val hasMetrics: Boolean, val json: String)
...
@Entity(tableName = "jobs", indices = [Index(value = ["sessionId", "scope", "scopeRevision", "kind", "clipKey", "stepOrdinal"], unique = true), Index("status")]) data class JobRow(@PrimaryKey val jobId: String, val sessionId: String, val scope: String, val scopeRevision: Long, val kind: String, val clipKey: String, val stepOrdinal: Int, val status: String, val createdAtMs: Long, val json: String)
...
@Entity(tableName = "session_summaries", primaryKeys = ["sessionId", "version"], indices = [Index("profileId")]) data class SummaryRow(val sessionId: String, val version: Int, val profileId: String, val completedAtMs: Long, val inputRevision: Long, val task: String, val dataOrigin: String, val measurementVersion: String, val lexicalVersion: String, val json: String)
...
@Dao abstract class WorkflowDao {
    ...
    @Query("SELECT ss.* FROM session_summaries ss INNER JOIN sessions s ON s.sessionId=ss.sessionId WHERE ss.profileId=:profileId AND ss.sessionId != :currentSessionId AND ss.task=:task AND ss.dataOrigin=:origin AND ss.measurementVersion=:method AND ss.lexicalVersion=:lexical AND ss.version=s.summaryVersion AND s.hasMetrics=1 AND s.createdAtMs < :before AND ss.version=(SELECT MAX(newer.version) FROM session_summaries newer WHERE newer.sessionId=ss.sessionId) ORDER BY ss.completedAtMs DESC, ss.sessionId DESC LIMIT 5")
    abstract suspend fun baseline(profileId: String, currentSessionId: String, task: String, origin: String, method: String, lexical: String, before: Long): List<SummaryRow>
