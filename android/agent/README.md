...
Observed host evidence on September 25, 2026: a temporary JVM-only harness using Kotlin 2.2.10, coroutines 1.10.1, serialization 1.8.0, and JDK 17 passed **93 core/agent/inference tests**, with no failures or skips. The **25 `CoordinatorTest` cases** use explicit in-memory store/model/audio/sponsor doubles. They cover observation, offline local comparison, duplicate commands, pause, saved-plan reconstruction, stale revisions progressing automatically, research approved during comparison completion, superseded ASR results, visible failures/timeouts, exact reviewed query/text exports, stale first text-approval previews, Events-only exports without duplicate Resume delivery, original summary chronology, consent revocation, and queued/in-flight memory refresh lifecycle behavior. They do not exercise Room, Whisper, loaded Liquid, actual process death, or an S24. Installer tests use tiny synthetic bytes; inference protocol tests use fake engines. Separate host native checks compile the real pinned llama.cpp library without weights; their results and limits are recorded in the native README.
