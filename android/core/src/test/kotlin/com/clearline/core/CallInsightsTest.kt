...
class CallInsightsTest {
    ...
    @Test fun negatedConcernsDoNotTriggerAndNoConcernHasExplicitFallback() {
        ...
        assertTrue(query("I don't have trouble finding words.").basis.fallback)
        assertTrue(query("I am not worried or lonely.").basis.fallback)
        assertTrue(query("I'm not feeling lonely or sad.").basis.fallback)
        assertEquals(CallConcern.LONELINESS, query("I am not worried, but I feel lonely.").basis.concern)
        assertEquals(CallConcern.MEMORY, query("I forget where I put things.").basis.concern)
        assertEquals(CallConcern.MEMORY, query("I do not know why I keep forgetting things.").basis.concern)
